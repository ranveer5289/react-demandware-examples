...
function demoReact1(){
	...
	const ItemComponent = require('~/cartridge/static/default/react/components/Item.js');

	const Item = React.createFactory(ItemComponent.Item);

	const itemProps = {initialCount : 7};
	const ItemHtml = ReactServer.renderToString(Item(itemProps));

	ISML.renderTemplate('/react/demo', {markup:ItemHtml});
}
